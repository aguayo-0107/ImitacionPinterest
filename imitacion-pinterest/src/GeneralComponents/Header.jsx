...
function Header() {
    const [nombre_usuario, set_nombre_usuario] = useState("Visitante");
    const navigate = useNavigate();
    useEffect(() => {
        const session = sessionStorage.getItem('user_session');
        set_nombre_usuario(session ? JSON.parse(session).nombre_usuario : "Visitante");
    }, [navigate]);
        ...
        <nav className="navbar navbar-expand-lg bg-body-tertiary shadow-lg">
            <div className="container-fluid">
                ...
                <NavLink className="nav-link pe-5" to="/profile">
                    <span className="me-2">{nombre_usuario}</span>
